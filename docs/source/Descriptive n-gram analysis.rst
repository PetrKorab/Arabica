...
The reviews are first made lowercase so that capital letters don't affect n-gram calculations (e.g., "Tree" is not
treated differently from "tree"). Next, the data is cleaned from punctuation, numbers, English stop words, and
additional strings ('br'). Finally, n-gram frequencies for each headline are calculated and
summed.
