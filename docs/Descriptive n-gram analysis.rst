...
**arabica_freq** method with *time_freq = 'ungroup'* provides a descriptive analysis for the most frequent n-grams specified by *'max_words'*.
...
The data subset contains lots of unnecessary strings. Arabica first makes text
lowercase so that capital letters don't affect n-gram calculations (e.g., "Tree"
is not treated differently from "tree"). In the next step, it **cleans the data**
from punctuation and numbers and removes the English set of stopwords. We also
remove a specific string ('br') as part of data cleaning:
